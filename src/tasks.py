...
@task(default=True, incrementable=["verbose"])
def help(c, verbose=0):
    ...
    print(
        "Try running `inv --help` for more information or `inv --list` to see a list of subcommands. "
    )
    print(
        "Some additional commands that are also installed and might be of use:  whois, speedtest, mtr, tcpdump, nmap, nc"
    )
...
@task(incrementable=["verbose"])
def docker_compose_version(c, verbose=0):
    try:
        client = docker.from_env()
    except Exception as e:
        raise invoke.Exit("ERROR: Docker connection failed.", e)
    ...
    print("Docker-compose version: ", info.get("ServerVersion"))
